...
int tiff_read_ifd(TIFFContext *c, IOContext *s, uint32_t offset) {
    ...
    // correct bitcount
    if (c->photo_interp == 32803) {
        if (c->samples_per_pixel != 3) {
            c->samples_per_pixel = 3;
            int bits_per_sample = c->bits_per_sample[0];
            int sz = c->samples_per_pixel * typesize_table[TIFF_DATA_TYPE_SHORT];
            free(c->bits_per_sample);
            c->bits_per_sample = (uint16_t*)malloc(sz);
            c->bitcount = 0;
            for (i = 0; i < c->samples_per_pixel; ++i) {
                c->bits_per_sample[i] = bits_per_sample;
                c->bitcount += c->bits_per_sample[i];
            }
        }
    }
...
TIFFContext* find_main_ctx(TIFFContext *ctx)
{
    TIFFContext *c;
    if (ctx->new_subfile_type == 0)
        return ctx;
    for (int i = 0; i < ctx->num_subifd; ++i) {
        if ((c = (TIFFContext*)(ctx->subifd[i].ctx_ptr)) != NULL)
            return c;
    }
    return NULL;
}
...
int parse_tiff(TIFFContext **ctx, char *filename)
{
    TIFFContext *c;
