...
int create_cfa_rggb_16bit(uint16_t **dst, uint16_t *src, int width, int height)
{
    int i, j, ret;
    uint16_t *p, *q;
...
int create_cfa_gbrg_16bit(uint16_t **dst, uint16_t *src, int width, int height)
{
    int i, j, ret;
    uint16_t *p, *q;
...
int create_cfa_grbg(uint8_t **dst, uint8_t *src, int width, int height)
{
    int i, j, ret;
    uint8_t *p, *q;
...
int create_cfa_grbg_16bit(uint16_t **dst, uint16_t *src, int width, int height)
{
    int i, j, ret;
    uint16_t *p, *q;
...
int create_cfa_bggr(uint8_t **dst, uint8_t *src, int width, int height)
{
    int i, j, ret;
    uint8_t *p, *q;
...
int create_cfa_bggr_16bit(uint16_t **dst, uint16_t *src, int width, int height)
{
    int i, j, ret;
    uint16_t *p, *q;
...
int create_cfa(uint8_t **dst, uint8_t *src, int width, int height, int sample_bits, int cfa_pattern)
{
    if (cfa_pattern == 0x01000201) { // gbrg
        if (sample_bits == 8)
            create_cfa_gbrg(dst, src, width, height);
        else if (sample_bits == 16)
            create_cfa_gbrg_16bit(dst, src, width, height);
    } else if (cfa_pattern == 0x01020001) { // grbg
        if (sample_bits == 8)
            create_cfa_grbg(dst, src, width, height);
        else if (sample_bits == 16)
            create_cfa_grbg_16bit(dst, src, width, height);
    } else if (cfa_pattern == 0x02010100) { // rggb
        if (sample_bits == 8)
            create_cfa_rggb(dst, src, width, height);
        else if (sample_bits == 16)
            create_cfa_rggb_16bit(dst, src, width, height);
    } else if (cfa_pattern == 0x00010102) { // bggr
        if (sample_bits == 8)
            create_cfa_bggr(dst, src, width, height);
        else if (sample_bits == 16)
            create_cfa_bggr_16bit(dst, src, width, height);
    }
}
...
int demosaicing(uint8_t **dst, uint8_t *src, int width, int height, int sample_bits, int cfa_pattern)
{
    // bayer转rgb
    int linebytes = width * 3;

    if (sample_bits == 8) {
        *dst = (uint8_t*)malloc(linebytes * height * sizeof(uint8_t));
        // calculate all g
        if (cfa_pattern == 0x01000201) { // gbrg
