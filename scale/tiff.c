...
int init_tiff_ctx(TIFFContext **s, uint8_t *data, int width, int height, int bitcount, int samples_per_pixel, int photo_interp/* = 2*/)
{
    ...
    for (i = 0; i < c->samples_per_pixel; ++i) c->bits_per_sample[i] = bitcount / samples_per_pixel;
    c->compression = 1;
...
void tiff_modify_entry(TIFFDirectory *td, uint16_t tag, uint32_t count, uint64_t *values)
{
    int size;
    int i, ret;
    TIFFEntry *entry = td->entries;

    for (i = 0; i < td->num_entries; ++i, entry++) {
        if (entry->tag == tag) {
            uint8_t *p = entry->is_offset ? entry->data : (uint8_t*)(&entry->value);
            entry->count = count;
            if (entry->type == TIFF_DATA_TYPE_ASCII) {
                *p = *((uint8_t*)values);
            } else if (entry->type == TIFF_DATA_TYPE_SHORT || entry->type == TIFF_DATA_TYPE_SIGNED_SHORT) {
                uint16_t *arr = (uint16_t*)p;
                for (i = 0; i < entry->count; ++i) arr[i] = values[i];
            } else if (entry->type == TIFF_DATA_TYPE_LONG || entry->type == TIFF_DATA_TYPE_SIGNED_LONG) {
                uint32_t *arr = (uint32_t*)p;
                for (i = 0; i < entry->count; ++i) arr[i] = values[i];
            } else if (entry->type == TIFF_DATA_TYPE_RATIONAL || entry->type == TIFF_DATA_TYPE_SIGNED_RATIONAL) {
                uint32_t *arr = (uint32_t*)p;
                for (i = 0; i < entry->count; ++i) arr[i] = values[i];
            } else if (entry->type == TIFF_DATA_TYPE_FLOAT) {
                float *arr = (float*)p;
                for (i = 0; i < entry->count; ++i) arr[i] = values[i];
            } else if (entry->type == TIFF_DATA_TYPE_DOUBLE) {
                double *arr = (double*)p;
                for (i = 0; i < entry->count; ++i) arr[i] = values[i];
            } else if (entry->type == TIFF_DATA_TYPE_BYTE || entry->type == TIFF_DATA_TYPE_SIGNED_BYTE) {
                //for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %02x ", i, entry.data[i]);
            }
            break;
        }
    }
...
{
    tiff_modify_entry(td, TIFFTAG_IMAGEWIDTH, 1, (uint64_t*)(&image_width));
}
void tiff_modify_image_height(TIFFDirectory *td, uint32_t image_height)
{
    tiff_modify_entry(td, TIFFTAG_IMAGELENGTH, 1, (uint64_t*)(&image_height));
}
void tiff_modify_default_crop_origin(TIFFDirectory *td, uint64_t default_crop_origin[2])
{
    tiff_modify_entry(td, TIFFTAG_DEFAULTCROPORIGIN, 2, default_crop_origin);
}
void tiff_modify_default_crop_size(TIFFDirectory *td, uint64_t default_crop_size[2])
{
    tiff_modify_entry(td, TIFFTAG_DEFAULTCROPSIZE, 2, default_crop_size);
}
void tiff_modify_active_area(TIFFDirectory *td, uint32_t active_area[4])
{
    uint64_t values[4];
    for (int i = 0; i < 4; ++i) values[i] = active_area[i];
    tiff_modify_entry(td, TIFFTAG_ACTIVEAREA, 4, values);
}
void tiff_modify_rows_per_strip(TIFFDirectory *td, uint32_t rows_per_strip)
{
    tiff_modify_entry(td, TIFFTAG_ROWSPERSTRIP, 1, (uint64_t*)(&rows_per_strip));
}
void tiff_modify_strip_offsets(TIFFDirectory *td, uint32_t *strip_offsets, int strips_per_image)
{
    uint64_t values[strips_per_image];
    for (int i = 0; i < strips_per_image; ++i) values[i] = strip_offsets[i];
    tiff_modify_entry(td, TIFFTAG_STRIPOFFSETS, strips_per_image, values);
}
void tiff_modify_strip_byte_counts(TIFFDirectory *td, uint32_t *strip_byte_counts, int strips_per_image)
{
    uint64_t values[strips_per_image];
    for (int i = 0; i < strips_per_image; ++i) values[i] = strip_byte_counts[i];
    tiff_modify_entry(td, TIFFTAG_STRIPBYTECOUNTS, strips_per_image, values);
}
int tiff_write_entry(TIFFEntry *entry, IOContext *s) {
    int64_t pos;
    int size, i, idx;
    int ret;
    TIFFDirEntry e;
    ...
    if ((size = io_write(s, &e, sizeof(TIFFDirEntry))) != sizeof(TIFFDirEntry)) {
        fprintf(stderr, "Error: failed to write entry at portion %ld\n", io_tell(s));
        return -1;
    }
    if (entry->is_offset) {
        pos = io_tell(s);
        size = entry->count * typesize_table[entry->type];
        io_seek(s, entry->value, SEEK_SET);
        io_write(s, (void*)entry->data, size);
        io_seek(s, pos, SEEK_SET);
    }
}

int tiff_rewrite_ifd(TIFFDirectory *td, IOContext *s) {
    uint16_t num = td->num_entries;
    int64_t pos;
    ...
    io_seek(s, td->offset, SEEK_SET);
    io_write(s, &num, 2);
    for (i = 0; i < num; ++i, entry++) {
        tiff_write_entry(entry, s);
    }
    if (td->image_data) {
        // fill image data
        pos = io_tell(s);
        uint8_t *p = td->image_data;
        for (i = 0; i < td->strips_per_image; ++i) {
            io_seek(s, td->strip_offsets[i], SEEK_SET);
            io_write(s, p, td->strip_byte_counts[i]);
            p += td->strip_byte_counts[i];
        }
        io_seek(s, pos, SEEK_SET);

        #if DEBUG_MODE
        static int fd = 0;
        if (td->new_subfile_type == 0 && !fd) {
            if ((fd = open("image_data_out.bin", O_WRONLY | O_CREAT | O_TRUNC, 0777)) == -1) {
                fprintf(stderr, "Error: failed to create file \"bayer.bin\"\n");
                return -1;
            }
            if ((ret = write(fd, td->image_data, td->image_size)) != td->image_size) {
    ...
    if (td->num_subifd) {
        // write subifds
        pos = io_tell(s);
        for (i = 0; i < td->num_subifd; ++i) {
            TIFFDirectory *ctx = (TIFFDirectory*)(td->subifd[i].ctx_ptr);
            ret = tiff_rewrite_ifd(ctx, s);
            if (ret < 0) {
                fprintf(stderr, "Error: failed to write ifd at portion %d\n", td->subifd[i].offset);
                return ret;
            }
        }
        io_seek(s, pos, SEEK_SET);
...
int tiff_rewrite(TIFFDirectory *td, char *filename)
{
    ...
    int i, ret;
    uint16_t byteorder = 0x4949, version = 42;
    uint32_t ifd_offset = 8, tag_value_offset = ifd_offset + 4, image_data_offset = ifd_offset + 4; // include 0x00000000
    IOContext *s;
    ...
    io_write(s, &byteorder, 2);
    io_write(s, &version, 2);
    io_write(s, &td->offset, 4);
    if ((ret = tiff_rewrite_ifd(td, s)) < 0) {
        fprintf(stderr, "Error: failed to write ifd at portion %d\n", ifd_offset);
        return ret;
    }
...
TIFFDirectory *tiff_alloc_context()
{
    TIFFDirectory *td;

    td = (TIFFDirectory *)calloc(1, sizeof(TIFFDirectory));
    td->num_entries = 0;
    td->entries_allocated = 32;
    td->entries = (TIFFEntry *)calloc(td->entries_allocated, sizeof(TIFFEntry));

    return td;
}

int tiff_read_entry(TIFFEntry *entry, IOContext *s) {
    int64_t pos;
    int size, i, idx;
    int ret;
    TIFFDirEntry e;
    if ((size = io_read(s, &e, sizeof(TIFFDirEntry))) != sizeof(TIFFDirEntry)) {
        fprintf(stderr, "Error: failed to read entry at portion %ld\n", io_tell(s));
        return -1;
    }
    entry->tag = e.tag;
    ...
    if (size <= TIFF_VALUE_SIZE) {
        ...
        pos = io_tell(s);
        io_seek(s, entry->value, SEEK_SET);
        io_read(s, (void*)entry->data, size);
        io_seek(s, pos, SEEK_SET);
    }
}

int tiff_parse_ifd(TIFFDirectory *td, IOContext *s) {
    uint16_t num;
    int64_t pos;
    TIFFEntry entry;
    int size, i, idx;
    ...
    for (idx = 0; idx < num; ++idx) {
        ...
        tiff_read_entry(&entry, s);
        if (++td->num_entries > td->entries_allocated) {
            td->entries_allocated <<= 1;
            td->entries = realloc(td->entries, td->entries_allocated * sizeof(TIFFEntry));
        }
        td->entries[idx] = entry;

        #if DEBUG_MODE
        fprintf(stdout, "entries[%d].tag: %04x, type: %d, count: %d", idx, entry.tag, entry.type, entry.count);
        if (!entry.is_offset) {
            fprintf(stdout, ", value: %d\n", entry.value);
        } else {
            fprintf(stdout, ", offset: %d\n", entry.value);
            if (entry.type == TIFF_DATA_TYPE_ASCII) {
                fprintf(stdout, "values: %s", entry.data);
            } else if (entry.type == TIFF_DATA_TYPE_SHORT || entry.type == TIFF_DATA_TYPE_SIGNED_SHORT) {
                uint16_t *arr = (uint16_t*)(entry.data);
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %d ", i, arr[i]);
            } else if (entry.type == TIFF_DATA_TYPE_LONG || entry.type == TIFF_DATA_TYPE_SIGNED_LONG) {
                uint32_t *arr = (uint32_t*)(entry.data);
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %d ", i, arr[i]);
            } else if (entry.type == TIFF_DATA_TYPE_RATIONAL || entry.type == TIFF_DATA_TYPE_SIGNED_RATIONAL) {
                uint32_t *arr = (uint32_t*)(entry.data);
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %d/%d ", i, arr[i * 2], arr[i * 2 + 1]);
            } else if (entry.type == TIFF_DATA_TYPE_FLOAT) {
                float *arr = (float*)(entry.data);
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %.6f ", i, arr[i]);
            } else if (entry.type == TIFF_DATA_TYPE_DOUBLE) {
                double *arr = (double*)(entry.data);
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %.6f ", i, arr[i]);
            } else if (entry.type == TIFF_DATA_TYPE_BYTE || entry.type == TIFF_DATA_TYPE_SIGNED_BYTE) {
                for (i = 0; i < entry.count; ++i) fprintf(stdout, "values[%d]: %02x ", i, entry.data[i]);
            }
        ...
        if (entry.tag == TIFFTAG_NEWSUBFILETYPE) {
            td->new_subfile_type = entry.value;
        } else if (entry.tag == TIFFTAG_IMAGEWIDTH) {
            td->image_width = entry.value;
        } else if (entry.tag == TIFFTAG_IMAGELENGTH) {
            td->image_length = entry.value;
        } else if (entry.tag == TIFFTAG_SAMPLESPERPIXEL) {
            td->samples_per_pixel = entry.value;
        } else if (entry.tag == TIFFTAG_BITSPERSAMPLE) {
            if (td->samples_per_pixel) assert(td->samples_per_pixel == entry.count);
            td->bits_per_sample = (uint16_t*)malloc(entry.count * sizeof(uint16_t));
            memcpy(td->bits_per_sample, (!entry.is_offset ? (&entry.value) : (void*)(entry.data)), entry.count * sizeof(uint16_t));
            td->bitcount = 0;
            for (i = 0; i < entry.count; ++i)
                td->bitcount += td->bits_per_sample[i];
            if (td->image_width && td->image_length) td->image_size = ((td->image_width * td->bitcount + 7) / 8) * td->image_length;
        } else if (entry.tag == TIFFTAG_ROWSPERSTRIP) {
            td->rows_per_strip = entry.value;
            td->strips_per_image = (td->image_length + td->rows_per_strip - 1) / td->rows_per_strip;
        } else if (entry.tag == TIFFTAG_STRIPBYTECOUNTS) {
            if (td->strips_per_image) assert(td->strips_per_image == entry.count);
            td->strip_byte_counts = (uint32_t*)malloc(entry.count * sizeof(uint32_t));
            if (entry.type == TIFF_DATA_TYPE_SHORT) {
                uint16_t *arr = (uint16_t*)(!entry.is_offset ? (&entry.value) : (void*)(entry.data));
                for (i = 0; i < entry.count; ++i)
                    td->strip_byte_counts[i] = arr[i];
            } else if (entry.type == TIFF_DATA_TYPE_LONG) {
                uint32_t *arr = (uint32_t*)(!entry.is_offset ? (&entry.value) : (void*)(entry.data));
                for (i = 0; i < entry.count; ++i)
                    td->strip_byte_counts[i] = arr[i];
            }
        } else if (entry.tag == TIFFTAG_STRIPOFFSETS) {
            if (td->strips_per_image) assert(td->strips_per_image == entry.count);
            td->strip_offsets = (uint32_t*)malloc(entry.count * sizeof(uint32_t));
            if (entry.type == TIFF_DATA_TYPE_SHORT) {
                uint16_t *arr = (uint16_t*)(!entry.is_offset ? (&entry.value) : (void*)(entry.data));
                for (i = 0; i < entry.count; ++i)
                    td->strip_offsets[i] = arr[i];
            } else if (entry.type == TIFF_DATA_TYPE_LONG) {
                uint32_t *arr = (uint32_t*)(!entry.is_offset ? (&entry.value) : (void*)(entry.data));
                for (i = 0; i < entry.count; ++i)
                    td->strip_offsets[i] = arr[i];
            }
        } else if (entry.tag == TIFFTAG_SUBIFDS) {
            td->subifd = (SubIFD*)malloc(entry.count * sizeof(SubIFD));
            td->num_subifd = entry.count;
            uint32_t *arr = (uint32_t*)(!entry.is_offset ? (&entry.value) : (void*)(entry.data));
            for (i = 0; i < entry.count; ++i)
                td->subifd[i].offset = arr[i];
        } else if (entry.tag == TIFFTAG_DEFAULTCROPORIGIN) {
            if (entry.type == TIFF_DATA_TYPE_SHORT) {
                uint16_t *arr = (uint16_t*)(&entry.value);
                td->default_crop_origin[0] = arr[0];
                td->default_crop_origin[1] = arr[1];
            } else if (entry.type == TIFF_DATA_TYPE_LONG) {
                uint32_t *arr = (uint32_t*)(entry.data);
                td->default_crop_origin[0] = arr[0];
                td->default_crop_origin[1] = arr[1];
            } else if (entry.type == TIFF_DATA_TYPE_RATIONAL) {
                uint64_t *arr = (uint64_t*)(entry.data);
                td->default_crop_origin[0] = arr[0];
                td->default_crop_origin[1] = arr[1];
            }
        } else if (entry.tag == TIFFTAG_DEFAULTCROPSIZE) {
            if (entry.type == TIFF_DATA_TYPE_SHORT) {
                uint16_t *arr = (uint16_t*)(&entry.value);
                td->default_crop_size[0] = arr[0];
                td->default_crop_size[1] = arr[1];
            } else if (entry.type == TIFF_DATA_TYPE_LONG) {
                uint32_t *arr = (uint32_t*)(entry.data);
                td->default_crop_size[0] = arr[0];
                td->default_crop_size[1] = arr[1];
        ...
        } else if (entry.tag == TIFFTAG_ACTIVEAREA) {
            if (entry.type == TIFF_DATA_TYPE_SHORT) {
                uint16_t *arr = (uint16_t*)(entry.data);
                for (i = 0; i < entry.count; ++i)
                    td->active_area[i] = arr[i];
            } else if (entry.type == TIFF_DATA_TYPE_LONG) {
                uint32_t *arr = (uint32_t*)(entry.data);
                for (i = 0; i < entry.count; ++i)
                    td->active_area[i] = arr[i];
            }
            if (td->default_crop_size[0] && td->default_crop_size[1])
                assert(td->default_crop_size[0] == td->active_area[3] - td->active_area[1] && td->default_crop_size[1] == td->active_area[2] - td->active_area[0]);
        } else if (entry.tag == TIFFTAG_PHOTOINTERP) {
            td->photo_interp = entry.value;
        } else if (entry.tag == TIFFTAG_CFAREPEATPATTERNDIM) {
            uint16_t *arr = (uint16_t*)(&entry.value);
            td->cfa_repeat_pattern_dim[0] = arr[0];
            td->cfa_repeat_pattern_dim[1] = arr[1];
        } else if (entry.tag == TIFFTAG_CFAPATTERN) {
            assert(entry.count >= 4);
            if (entry.is_offset) {
                td->cfa_pattern = entry.data[0];
                td->cfa_pattern |= (entry.data[1] << 8);
                td->cfa_pattern |= (entry.data[2] << 16);
                td->cfa_pattern |= (entry.data[3] << 24);
            } else {
                td->cfa_pattern = entry.value;
            }
        }
    }
    if (td->photo_interp == 32803) { // RAW image data
        td->samples_per_pixel = 1;
    }
    if (td->image_size && td->strips_per_image) {
        pos = io_tell(s);
        td->image_data = (uint8_t*)malloc(td->image_size);
        uint8_t *p = td->image_data;
        for (i = 0; i < td->strips_per_image; ++i) {
            io_seek(s, td->strip_offsets[i], SEEK_SET);
            io_read(s, p, td->strip_byte_counts[i]);
            p += td->strip_byte_counts[i];
        }
        io_seek(s, pos, SEEK_SET);
        ...
        static int fd = 0;
        if (td->new_subfile_type == 0 && !fd) {
            if ((fd = open("image_data.bin", O_WRONLY | O_CREAT | O_TRUNC, 0777)) == -1) {
                fprintf(stderr, "Error: failed to create file \"bayer.bin\"\n");
                return -1;
            }
            if ((ret = write(fd, td->image_data, td->image_size)) != td->image_size) {
    ...
    if (td->num_subifd) {
        pos = io_tell(s);
        for (i = 0; i < td->num_subifd; ++i) {
            TIFFDirectory *ctx = tiff_alloc_context();
            ctx->offset = td->subifd[i].offset;
            ret = tiff_parse_ifd(ctx, s);
            if (ret < 0) {
                fprintf(stderr, "Error: failed to read ifd at portion %d\n", td->subifd[i].offset);
                return ret;
            }
            td->subifd[i].ctx_ptr = ctx;
        }
        io_seek(s, pos, SEEK_SET);
    }
    return ret;
